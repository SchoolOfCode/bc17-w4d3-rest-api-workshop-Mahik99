...
app.post("/astronauts", async (req, res) => {
  try {
    const data = await createAstronaut(req.body);
    res.json({
      success: true,
      payload: data,
    });
...
app.get("/astronauts/:id", async (req, res) => {
  try {
    ...
    const astros = await getAstronautById(req.params.id);
    res.json({
      success: true,
      payload: astros,
    });
...
app.put("/astronauts/:id", async (req, res) => {
  try {
    const astros = await replaceAstronautById(req.params.id, req.body);
    res.json({
      success: true,
      payload: astros,
    });
...
app.delete("/astronauts/:id", async (req, res) => {
  try {
    const astros = await deleteAstronautById(req.params.id);
    res.json({
      success: true,
      payload: astros,
    });
